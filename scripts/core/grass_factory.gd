...
# Fonction pour initialiser une graine basée sur la position
static func set_seed_from_position(chunk_pos: Vector3):
	var seed_value = int(chunk_pos.x * 1000 + chunk_pos.z)
	seed(seed_value)
...
static func generate(
	...
	p_sway_yaw: Vector2,
	chunk_pos: Vector3,
	chunk_dimensions: float) -> Array:
	var spawns: Array = []
	
	# Initialiser la graine pour rendre la génération déterministe
	set_seed_from_position(chunk_pos)
	
	# Si aucun mesh n'est fourni, générer une grille uniforme dans le chunk
	if !p_mesh:
		var half_size = chunk_dimensions * 0.5
		var _step: float = 1.0 / sqrt(p_density)
		var blades_per_chunk: int = int(round(chunk_dimensions * chunk_dimensions * p_density))
		for i in range(blades_per_chunk):
			var local_pos = Vector3(
				randf_range(-half_size, half_size),
				0.0,
				randf_range(-half_size, half_size)
			)
			var position = chunk_pos + local_pos
