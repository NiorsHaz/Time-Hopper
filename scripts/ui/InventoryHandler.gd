...
@export var InventorySlotPrefab : PackedScene = preload("res://scenes/ui/InventorySlot.tscn")
...
func remove(name: String, number: int) -> void:
	var filtered = InventorySlots.filter(func(inventory): 
		return inventory.SlotFilled and inventory.SlotData and inventory.SlotData.name == name)
		
	var count = min(number, filtered.size())
	for i in range(count):
		var slot = filtered[i]
		InventorySlots[slot.InventorySlotID].FillSlot(null, false)
	
		
func filtre_slots(name: String, return_array: bool = false) -> Variant:
	var filtered = InventorySlots.filter(func(inventory): return inventory.SlotFilled == true)
	if filtered.size() > 0:
		filtered = filtered.filter(func(slot): return slot.SlotData.name == name)
		if return_array:
			return filtered
...
func PickupItem(item : ItemData):
	...
	if (!foundSlot):
		var newItem = item.ModelPrefab.instantiate() as Node3D
...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	var newItem = InventorySlots[data["ID"]].SlotData.ModelPrefab.instantiate() as Node3D
